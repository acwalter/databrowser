                    ...
                    if (data.total_rows > 0) {
                        ...
                        $.getJSON(url, function(data) { // Try to find a match in the MAT
                            if (data.total_rows > 0) {
                                ...
                                message += "<br /><br /><strong><a href='javascript:void(0)' class='identify_select' data-matid='" + data.rows[0].row.objectid + "' onclick='locationFinder({ \"gid\": " + data.rows[0].row.objectid + ", \"type\": \"ADDRESS\"});'>Select this Location</a></strong>";
                                $.publish("/layers/addmarker", [{
